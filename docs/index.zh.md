...
Gabaix et al. 只拟合单一截面。我们需要的是一*序列*嵌入，它随数据移动、且处在*同一套连贯的坐标系*中。对每个季度独立拟合（即**朴素（naive）**做法）会令隐空间坐标轴的旋转/反射/置换无法识别，于是季度间的变化就把真实的资产层面变化与优化坐标系漂移混淆在一起。

我们的**耦合（coupled）**做法：在前 80% 的持仓观测上预训练一个基模型（语料按序列条数沿时间顺序切分，截点落在 2023Q1：前 73 个季度用于预训练，其后 6 个季度用于逐季度微调），并令每个季度的微调都从这个共享锚点初始化——这是本文核心的方法论承诺。两条初始化耦合贯穿整条流水线（W2V-to-BERT 与 PT-to-FT）。参见 [方法：论文 ↔ 代码](explanation/method.md)。

<p align="center"><img src="figures/pipeline.svg" alt="Pretrain-finetune pipeline" width="720"></p>

## 本仓库交付什么

一套覆盖完整嵌入流水线的训练框架：三种架构（推荐系统 SVD、Word2Vec、BERT）、耦合的预训练-微调流程，以及配套的数据工具。产出是一个逐季度的 embedding CSV 序列——每只股票每季度一个向量，全部处在同一套连贯的坐标系中。配套论文研究这些嵌入对公司估值与收益相关性的解释力。
