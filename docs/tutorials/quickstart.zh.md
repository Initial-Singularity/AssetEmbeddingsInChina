...
1. **W2V-PT**——在合成的组合“句子”上训练 Skip-gram Word2Vec。
2. **W2V-FT**——在微调期数据上继续训练 W2V，从预训练向量出发（**PT→FT**）。
3. **BERT-PT**（预训练）——在汇总的历史数据上训练 BERT，其嵌入层由 W2V-PT 矩阵热启动（**W2V→BERT**）。
4. **BERT-FT**（微调）——按“季度”精修 BERT，编码器从预训练模型初始化（**PT→FT**），嵌入层由 W2V-FT 热启动（**W2V→BERT**）。
